#include "kernel.hpp"
#include "ACA.hpp"
#include "HODLRTree.hpp"
#include "gmres.hpp"

class HODLR3D {
public:
	HODLRTree<userkernel>* A;
	HODLR3D(int cubeRootN, int nParticlesInLeafAlong1D, double L, int TOL_POW, int Qchoice) {
		...
		A	=	new HODLRTree<userkernel>(mykernel, cubeRootN, nLevels, nParticlesInLeafAlong1D, L, TOL_POW);
...
int main(int argc, char* argv[]) {
  ...
  for (it = s.begin(); it != s.end(); it++) {
	...
	start	=	omp_get_wtime();
	G->gmres(H, true_Ax, maxIterations, GMRES_threshold, HODLR_x, GMRES_residual, noOfIterations, e);
	end		=	omp_get_wtime();
