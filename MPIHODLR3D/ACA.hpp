...
public:
	...
	LowRank(kernel* K, int tol_pow, std::vector<int>& row_indices, std::vector<int>& col_indices) {
		this->K					=	K;
	...
	// 	std::vector<Eigen::VectorXd> AcVec;
	// 	std::vector<Eigen::VectorXd> ArVec;
	//
	...
	// 		}
	// 		if (l_local == N1) {
	// 			Ac = Eigen::MatrixXd(N1,computed_rank);
	// 			Ar = Eigen::MatrixXd(computed_rank,N2);
	// 			return;
	// 		}
	// 		v=row;
	...
	// void ACA_only_nodes(std::vector<int>& row_bases, std::vector<int>& col_bases, int &computed_rank, Eigen::MatrixXd &Ac, Eigen::MatrixXd &Ar, Eigen::MatrixXd &L, Eigen::MatrixXd &R) {
	// void ACA_only_nodes(std::vector<int>& row_bases, std::vector<int>& col_bases, int &computed_rank, Eigen::MatrixXd &Ac, Eigen::MatrixXd &Ar, Eigen::MatrixXd &L, Eigen::MatrixXd &R) {
	// void ACA_only_nodes(std::vector<int>& row_bases, std::vector<int>& col_bases, int &computed_rank, Eigen::MatrixXd &Ac, Eigen::MatrixXd &Ar) {
		int col_index;
		int row_index;
	...
	// void ACA_only_nodes(std::vector<int>& row_bases, std::vector<int>& col_bases, int &computed_rank, Eigen::MatrixXd &Ac, Eigen::MatrixXd &Ar) {
		...
		for(int l= 0; l < N2; l++) {
			...
			// }
			// U = Eigen::MatrixXd (N1,computed_rank);
			// V = Eigen::MatrixXd(computed_rank,N2);
			// for (size_t i = 0; i < computed_rank; i++) {
			// 	U.col(i) = Uvec[i];
			// 	V.row(i) = Vvec[i];
			// }
